...
export function DnsImpactPreviewCard({ preview }: Props) {
  ...
  const iconClass =
    level === "high" ? "text-destructive" : level === "medium" ? "text-warning-foreground" : "text-success";

  return (
    <div className={cn("rounded-xl border p-5 sm:p-6", tone)} role="status" aria-label="DNS impact preview">
      <div className="flex gap-3">
        ...
        <div className="min-w-0 flex-1 space-y-2">
          ...
          <div className="flex flex-wrap gap-2 text-xs">
            <Badge variant="outline" className={cn(preview.emailAtRisk ? "border-warning/30 bg-warning-soft" : "bg-muted")}>
              <Mail className="mr-1 size-3" />
              {preview.emailAtRisk ? "Business email may be affected" : "No business email at risk"}
            </Badge>
            <Badge variant="outline" className={cn(preview.existingWebsiteAtRisk ? "border-warning/30 bg-warning-soft" : "bg-muted")}>
              <Globe className="mr-1 size-3" />
              {preview.existingWebsiteAtRisk ? "Existing website at risk" : "No existing website at risk"}
            </Badge>
          </div>
          ...
          {preview.recordsToPreserve.length > 0 ? (
            <div className="rounded-lg border border-border bg-card/60 p-3">
              ...
              {(preview.emailAtRisk || preview.recordsToPreserve.some((r) => r.toLowerCase().includes("mx") || r.toLowerCase().includes("spf"))) ? (
                <p className="mt-2 text-xs leading-relaxed text-muted-foreground">
                  ...
                  Keep them separate from website records.
                </p>
              ) : null}
            </div>
          ) : null}
          ...
          <p className="text-xs text-muted-foreground">
            Manual check required — verify each item yourself before editing DNS. This app does not change DNS for you.
          </p>
        </div>
      </div>
    </div>
