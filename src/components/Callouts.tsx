...
const toneStyles: Record<Tone, { wrap: string; icon: typeof Info; iconClass: string }> = {
  ...
  warning: { wrap: "bg-warning-soft border-warning/40", icon: AlertTriangle, iconClass: "text-warning-foreground" },
  success: { wrap: "bg-success-soft border-success/35", icon: CheckCircle2, iconClass: "text-success" },
  danger: { wrap: "bg-destructive-soft border-destructive/35", icon: AlertTriangle, iconClass: "text-destructive" },
};
...
export function OwnershipWarningCard({ className }: { className?: string }) {
  return (
    <div className={cn("rounded-xl border border-warning/40 bg-warning-soft p-5", className)}>
      <div className="flex gap-3">
        <ShieldCheck className="mt-0.5 size-6 shrink-0 text-warning-foreground" aria-hidden="true" />
        <div className="space-y-2">
          <p className="font-display text-lg font-semibold">Important: keep ownership in your name</p>
          <ul className="space-y-1.5 text-sm text-muted-foreground">
            ...
            <li>Keep a written record of who can access domain, hosting, email, billing and analytics.</li>
