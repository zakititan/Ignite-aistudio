...
export const Route = createFileRoute("/status")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Current status of the guides, plan saving and sign-in, plus how to check whether the problem is your own provider.",
      },
      { property: "og:title", content: "Service status" },
      { property: "og:description", content: "All systems and how to tell if an outage is ours or your provider's." },
    ],
  }),
...
> = {
  operational: { label: "Operational", icon: CheckCircle2, className: "text-success-foreground bg-success-soft" },
  degraded: { label: "Degraded", icon: AlertTriangle, className: "text-warning-foreground bg-warning-soft" },
...
function StatusPage() {
  ...
  return (
    ...
    >
      <Callout tone={allGood ? "success" : "warning"} title={allGood ? "All systems operational" : "Some systems are affected"}>
        {allGood
      ...
      <section className="surface-panel divide-y divide-border">
        ...
        <ul>
          {SERVICE_STATUSES.map((s) => {
            ...
            const Icon = ui.icon;
            return (
              <li key={s.id} className="flex flex-wrap items-center justify-between gap-3 border-t border-border p-5 sm:px-6">
                <div>
      ...
      <ContentSection title="Is it us, or your provider?">
        <p>
          This page only covers Launch My Business Online. Your website, domain and email are run by other companies,
          and most outages people meet are theirs, not ours. To check:
        </p>
      ...
      <ContentSection title="Recent history">
        <p className="text-sm text-muted-foreground">
          No incidents recorded in the last 90 days. Incident history is a placeholder while monitoring is being set
          up.
        </p>
      </ContentSection>
    </ContentPageLayout>
